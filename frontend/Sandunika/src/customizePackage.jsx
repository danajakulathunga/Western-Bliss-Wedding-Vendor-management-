...
function CustomizePackage() {
    ...
    const options = {
        ...
        catering: [
            ...
            { label: "Basic Catering-Anglis", price: 80000 },
            { label: "Basic Catering-Rathna Catering", price: 100000 },
        ],
        music: [
        ...
        decorations: [
            ...
            { label: "Normal Decorations-Lassana flora", price: 50000 },
            { label: "Simple Decorations-new flora", price: 20000 },
        ],
        Vehicle: [
            { label: "Luxury Vehicle-Benz", price: 100000 },
            { label: "Luxury Vehicle-Axio", price: 50000 },
            { label: "Luxury Vehicle-Fit", price: 20000 },
        ]
    };
    ...
    const handleBookPackage = async () => {
        ...
        if (!user.phoneNumber) {
            alert('Phone number is required. Please update your profile.');
            return;
        }

        try {
            // First create a custom package
            const packageData = {
                packageName: 'Custom Package',
                price: calculateFinalPrice(),
                image: 'custom-package.jpg',
                features: Object.entries(customPackage)
                    .filter(([key, value]) => key !== 'price' && value)
                    .map(([key, value]) => value)
            };

            const packageResponse = await fetch('http://localhost:5000/api/packages', {
                method: 'POST',
                headers: {
            ...
            const packageId = packageResult.data._id;

            // Then create the booking with the package ID
            const bookingData = {
                packageId,
    ...
    return (
        <div className="w-screen h-screen bg-pink-50 flex flex-col justify-center items-center p-10 overflow-auto">
            ...
            <div className="bg-white shadow-2xl rounded-xl p-8 w-full max-w-4xl border border-pink-200 relative">
                ...
                <div className="sticky bottom-0 left-0 w-full bg-white py-4 flex justify-center gap-4 border-t border-pink-200 mt-6">
                    <button 
                        className="text-black px-8 py-3 rounded-lg shadow-md text-lg font-bold hover:bg-pink-700 transition-all bg-pink-600 text-white"
